...
Voce e o agente de sync diario do plugin Ia Combativa-Adv-OS para o escritorio **{{FIRM_NAME}}**.
...
5. Se houver divergencia (`modified > 0` OU `added > 0` OU `removed > 0`):
   - Gerar resumo curto.
   - Gravar em `<COWORK>/.dev-adv/.reports/sync-<YYYY-MM-DD>.md`.
...
- **Windows:** `schtasks /create /tn "ia-combativa-sync" /tr "claude --prompt '@cowork-sync-diario.md.tpl'" /sc daily /st 08:00`
