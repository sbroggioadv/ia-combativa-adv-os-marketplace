...
2. Listar arquivos `.txt` / `.md` novos nas ultimas N execucoes (manter log em `<COWORK>/.dev-adv/.hook-state.json` campo `meeting-triage.processed_files`).
...
- **Windows (a cada 30min):** `schtasks /create /tn "ia-combativa-meeting" /tr "..." /sc minute /mo 30`
