...
3. Gerar tabela consolidada em `<COWORK>/.dev-adv/.reports/email-triage-<YYYY-MM-DD>.md`.
...
SUGESTAO DE RESPOSTAS (rascunhos — nao enviados):
  Ver <COWORK>/.dev-adv/.drafts/email-<ts>-<thread-id>.md
```
...
- Rascunhos de resposta sao gravados em `<COWORK>/.dev-adv/.drafts/` — essa pasta deve estar em `.gitignore`.
...
- **Windows:** `schtasks /create /tn "ia-combativa-email" /tr "..." /sc daily /st 08:00`
