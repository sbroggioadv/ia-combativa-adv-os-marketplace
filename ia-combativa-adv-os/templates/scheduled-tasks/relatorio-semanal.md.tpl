...
2. **`<COWORK>/.dev-adv/.memory-evolver-pending.json`** — edicoes significativas aguardando consolidacao.
3. **Integracoes declaradas** em `<COWORK>/.dev-adv/persona.md` — consultar SE e somente SE declaradas:
...
- Ao final, gravar o relatorio em `<COWORK>/.dev-adv/.reports/semanal-<YYYY-MM-DD>.md`.
...
- **Windows (Task Scheduler):**
  ```powershell
  schtasks /create /tn "ia-combativa-relatorio-semanal" /tr "claude --prompt '@relatorio-semanal.md.tpl'" /sc weekly /d MON /st 08:00
  ```
- **macOS (launchd):** criar `~/Library/LaunchAgents/com.ia-combativa.relatorio-semanal.plist` com StartCalendarInterval Weekday=1, Hour=8.
