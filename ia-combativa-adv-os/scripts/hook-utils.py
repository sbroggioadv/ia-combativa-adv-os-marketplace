...
Funcoes:
- find_cowork(path)       → descobre COWORK root procurando .dev-adv/ acima
- hook_state_load/save    → gerencia .dev-adv/.hook-state.json (debounce anti-flap)
...
def find_cowork(start: Path) -> Path | None:
    """Procura uma pasta ancestral de `start` que contenha <dir>/.dev-adv/cowork-state.json."""
    start = start.resolve()
