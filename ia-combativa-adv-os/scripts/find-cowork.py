...
3. <CWD>/.dev-adv/cowork-state.json (subindo ate 5 niveis)
4. <CWD>/.claude/settings.local.json campo env.COWORK_PATH
5. ~/.config/ia-combativa-adv-os/active-cowork.json campo cowork_path
...
def _has_state(p: Path) -> bool:
    return (p / ".dev-adv" / "cowork-state.json").exists()
...
def _try_active_cowork() -> Path | None:
    for cfg in (
        Path.home() / ".config" / "ia-combativa-adv-os" / "active-cowork.json",
        Path.home() / ".ia-combativa-adv-os" / "active-cowork.json",
    ):
        if cfg.exists():
            try:
